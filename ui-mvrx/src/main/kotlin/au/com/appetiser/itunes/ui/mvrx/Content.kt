 ...
 */
sealed class Content<out T>(val complete: Boolean, val shouldLoad: Boolean) {
...
object Uninitialized : Content<Nothing>(complete = false, shouldLoad = true), Incomplete

data class Next<out T>(private val value: T) : Content<T>(complete = true, shouldLoad = false) {
...
class Loading<out T> : Content<T>(complete = false, shouldLoad = false), Incomplete {
    override fun equals(other: Any?) = other is Loading<*>

    override fun hashCode() = "Loading".hashCode()
}

data class Success<out T>(private val value: T) : Content<T>(complete = true, shouldLoad = false) {
...
data class Fail<out T>(val error: Throwable) : Content<T>(complete = true, shouldLoad = true) {
